...
The development-only harness accepts `?visual=main-menu`, `?visual=run-hud`, or `?visual=stress` (200 enemies and 400 projectiles). Add `&baseline=pre` only when recapturing the legacy comparison images; the normal harness renders the current visual contract.
